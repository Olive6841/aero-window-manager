...
__int64 CTLW_ValidateVisual_Hook(BYTE* pThis)
{
    // TO BE OVERHAULED ONCE/IF VALIDATEVISUAL HAS BEEN REWRITTEN
    ...
    if (!rv && /**(HWND*)(windowData + CWD_HWND) && (*(pThis + wState1) & STYLE_THICKFRAME) == 0 &&*/ hasClient)
    {
        ...
        if ((*(pThis + wState1) & 2) == 0 || !isEqual)
        {
            // COMPATIBILITY: REMEMBER TO UPDATE THE 2 HERE TO BE COMPATIBLE WITH EVERY VERSION
            HRGN rgn = CreateRectRgnIndirect(&rect);
            fprintf(stream, "i have set the region\n");
            if (!SetWindowRgnEx(*(HWND*)(windowData + CWD_HWND), rgn, 1))
            {
                // I NEED TO GET SYMBOLS FOR USER32.DLL TO USE SETWINDOWRGNEX (and hope it works)
                if (rgn)
                {
                    DeleteObject(rgn);
                }
...
{
    if (!*(long long*)(pThis + CTLW_ButtonVisuals + 8 * buttonId))
    {
        // if this button doesn't exist on the window, return.
        return 0;
    }
